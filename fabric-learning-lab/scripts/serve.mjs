...
server.listen(port, () => {
  console.log(`Fabric Learning Lab is available at http://localhost:${port}`);
  console.log(`Serving ${root}`);
});

server.on("error", (error) => {
  if (error.code === "EADDRINUSE") {
    console.error(`Port ${port} is already in use. Set another port, for example: $env:PORT=5175; npm run serve`);
    process.exitCode = 1;
    return;
  }
